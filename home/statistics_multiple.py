...
def GetStatistics(chromosome, CNV_start, CNV_end, phenotypes, zoom):
    ...
    for j in range(4, len(genes)):
            ...
            for k in range(len(gene_phenotypes)):
                if gene_phenotypes[k] in phenotype_list:
                    matched_actual.append(str(genes[j].name.strip()) + "(" + str(gene_phenotypes[k]) + ")")
        
    number_of_matches_actual = len(genes_with_matches)

    actual_weighted = weighted_score
    actual_unique_weighted = unique_weighted_score
    lower_weighted_score = 0
    lower_unique_weighted = 0
    ...
    for i in range(500):
        ...
        if unique_weighted_score < actual_unique_weighted:
            lower_unique_weighted += 1

    return str(chromosome)+"\t"+str(start)+"\t"+str(end)+"\t"+\
        str(number_of_matches_actual)+"\t"+str(total_phenotype_score_actual)+"\t"+str(lower_weighted_score)+\
        "\t"+str(unique_matches_actual)+"\t"+str(lower_unique_weighted)+"\t"+str(matched_actual)+"\r\n"
...
def generate_CNV(CNV_length):
    ...
    return(chromosome, start, start+CNV_length)
...
def GetTADs(chromosome, CNV_start, CNV_end, phenotypes, zoom):
            ...
            for k in range(len(gene_list[i].phenotypes)):
                if phenotype_list[j] == gene_list[i].phenotypes[k]:
                    ...
                    if phenotype_list[j] not in unique_matches:
                        unique_weighted_score += 20/gene_list[i].weights[k]
    ...
    return(my_array_of_genes, genes_with_matches, total_phenotype_score, weighted_score, unique_weighted_score,
           len(unique_matches))
