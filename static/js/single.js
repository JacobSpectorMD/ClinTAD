...
function set_svg_info(build, current_coordinates) {
    $('#svg-build-info').html(build);
...
function create_svg(data) {
    const coordinates = 'chr' + data.chromosome + ':' + data.cnv_start + '-' + data.cnv_end;
    set_svg_info(data.build, coordinates)
    $('#container').html('');
...
function handle_track(data, track, scale, num_tracks, start_coord, end_coord) {
    const build = track.build.split('/')[0];
    $('#svg-build-info').html(build)
    let track_svg;
...
function draw_cnv_genes(data, scale, num_tracks) {
    ...
    for (var i = 0; i < genes.length; i++) {
        var stroke_color = "#8F8470";
        if (genes[i].phenotype_score > 0) {
            stroke_color = "orange"
        }
        ;
        ...
        if (scale(genes[i].end) > end_point) {
            end_point = scale(genes[i].end);
        }
        ...
        svg.append("line")
            ...
            .on("click", function () {
                ...
                // Adds information about the clicked gene to the Selected Data panel
                var gene_number = this.getAttribute("data-gene_number");
                var geneInfo = data_to_display[gene_number];

                // For each attribute of the gene, creates a column showing the name of the attribute (e.g. gene name)
                // and the value of that attribute (e.g. TP53)
                Object.keys(geneInfo).forEach(function (key) {
                    if (key != "Phenotypes" && key != "Matches") {
                        $('#patient-data-display').append(`<div>${key}:</div><div>${geneInfo[key]}</div>`);
                    }
                });

                // Add the HPOs that match to patient phenotype for the clicked gene
                $('#patient-data-display').append(`<div>Matches:</div>`);
                const hpoMatchDiv = $('<div></div>');
                const match_ids = [];
                geneInfo['Matches'].forEach(function (hpo) {
                    match_ids.push(hpo.hpo);
                    $(hpoMatchDiv).append(`
                        <span data-hpoid="${hpo.hpo}" class="match" title="${hpo.hpo}">
                            ${hpo.name}
                        </span>
                    `)
                })
                $('#patient-data-display').append(hpoMatchDiv);

                // Add the HPOs that do not match to patient phenotype for the clicked gene
                $('#patient-data-display').append(`<div>Phenotypes:</div>`);
                const phenotypesDiv = $('<div></div>');
                geneInfo['Phenotypes'].forEach(function (hpo) {
                    let matchClass = "hpo";
                    if ($.inArray(hpo.hpo, match_ids) != -1) {
                        matchClass = "match";
                    }

                    $(phenotypesDiv).append(`
                        <span data-hpoid="${hpo.hpo}" class="${matchClass}" title="${hpo.hpo}">${hpo.name}</span>
                    `)
                })
                $('#patient-data-display').append(phenotypesDiv);
...
function draw_cnvs(data, track, scale, num_tracks) {
    ...
    for (var i = 0; i < variants.length; i++) {
        if (variants[i].subtype == 'gain') {
            var stroke_color = 'blue'
        } else {
            var stroke_color = 'red'
        }

        // Make rare variants less opaque
        var opacity = (variants[i].frequency >= 1) ? 1 : 0.3;

        // End of gene rectangle, use either the start plus 10 pixels or for longer genes just scale the line
        var end_point = scale(variants[i].outer_start) + 10;
        if (scale(variants[i].outer_end) > end_point) {
            end_point = scale(variants[i].outer_end);
        }

        let row = 0;
        let overlap = true;
        while (overlap) {
            if (i == 0) { // For the first element, just add it
                last_end_point[0] = end_point;
                overlap = false;
            } else if (row >= last_end_point.length) {
                // For the first element in a row, just add it
                last_end_point.push(end_point);
                overlap = false;
            } else if (scale(variants[i].outer_start) >= last_end_point[row] + 3) {
                // If the element does not overlap something in current row, add it
                last_end_point[row] = end_point;
                overlap = false;
            } else if (scale(variants[i].outer_start) < last_end_point[row] + 3) {
                // If the element overlaps something in the current row, go to the next row
                row += 1;
            }
        }
        ...
        svg.append("line")
            ...
            .on("click", function () {
                ...
                //Adds information about the clicked copy number variant to the Selected Data panel
                var gene_number = this.getAttribute("data-gene_number");
                var variantInfo = data_to_display[gene_number];
                Object.keys(variantInfo).forEach(function (key) {
                    $('#patient-data-display').append(`<div>${key}:</div><div>${variantInfo[key]}</div>`);
                });
            });
    }
...
function draw_track(data, track, scale, num_tracks) {
    ...
    for (var i = 0; i < elements.length; i++) {
        ...
        if (scale(elements[i].end) > end_point) {
            end_point = scale(elements[i].end);
        }
        ...
        svg.append("line")
            ...
            .on("click", function () {
                ...
                //Adds information about the clicked object to the Selected Data panel
                var gene_number = this.getAttribute("data-gene_number");
                const elementInfo = data_to_display[gene_number];
                const elementInfoDiv = $('<div></div>');
                Object.keys(elementInfo).forEach(function (key) {
                    if (key != "Phenotypes" && key != "Matches") {
                        $(elementInfoDiv).append(`
                            <div>${key}: </div><div>${elementInfo[key]}</div>
                        `)
                    }
                });
                $('#patient-data-display').append(elementInfoDiv);
...
function draw_boundaries(data, svg, scale, tads, y1, start_coord, end_coord, boundaries_only = true) {
    ...
    for (var i = 0; i < tads.length; i++) {
        ...
        if (i < tads.length - 1) {
            if (right == tads[i + 1]['start']) {
                draw_dashed(right, "r", false)
            }
            ;
            if (right != tads[i + 1]['start']) {
    ...
    if (data.minimum['type'] == 'chromosome') {
        draw_dashed(start_coord, "l", true);
    }
    if (data.maximum['type'] == 'chromosome') {
        draw_dashed(end_coord, "r", true);
    }
}
